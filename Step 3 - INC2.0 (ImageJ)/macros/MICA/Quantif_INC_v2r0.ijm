...
//	print("[3] Add New "+StrType[2]);
	...
	run("Action Bar","/plugins/ActionBar/INC_v1r0.txt");
	wait(50);
}
...
// on est ok
				SelectedType=false;
				run("Action Bar","/plugins/ActionBar/INC_Select_v1r0.txt");
				while(!SelectedType) {
					wait(50);
				}
				roiManager("Show All with labels");
...
macro "Add To Type I"{
	...
	while (isOpen('INC_Select_1r0')){
		eval("js","frame = WindowManager.getWindow('INC_Select_1r0');frame.close;")
	}
	SelectedType=true;
}
